...
export const extractPinAndSignature = (rawText: string): { pins: string[], signature: string, text: string } | null => {
...
export const getHTMLContent = async (url: string) => {
  ...
  if(html.indexOf('INTERNAL SERVER ERROR') > 0) throw new Error('Invalid request')
  return html.trim();
};
...
export const getJSONData = async ({pin, signature}:{pin: string, signature: string}) => {
